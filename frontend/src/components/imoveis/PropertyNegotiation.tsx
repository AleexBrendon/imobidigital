      ...
      <div className="grid max-h-[90px] grid-cols-3 gap-x-5 gap-y-2 overflow-y-auto border-t border-white/10 pt-3 pr-2 no-scrollbar">
        {negotiations.map((item) => (
          <div
            key={item.id}
            onClick={() => onEdit(item)}
            className="relative flex cursor-pointer items-center gap-2 rounded-lg px-1.5 py-1.5 hover:bg-white/5"
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-medium text-slate-100">
                {item.name ?? "Negociação"} - {item.stage}
              </p>
            ...
            {openMenuId === item.id && (
              <div
                ...
                className="absolute right-0 top-8 z-30 w-36 rounded-xl border border-white/10 bg-[#142438] p-1 shadow-2xl"
              >
                <button
                  type="button"
                  onClick={() => {
